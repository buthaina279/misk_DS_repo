...
HotelTable <- Hotel_bookings %>% 
            group_by(hotel) %>%
             summarise(n = length(Hotel_bookings),
...
```{r}
t.test(adr ~ hotel, data = Hotel_bookings, var.equal = TRUE)
```
...
ggplot(aes(x = stays_in_week_nights, y = adr)) +
  geom_point(position = posn_j, shape = ".", alpha = 0.25) +
  # stat_summary(fun.data = mean_sdl, 
               #fun.args = list(mult = 1), col = "red") +
  geom_smooth(method = "lm", se = FALSE) +
  NULL
```
- the null hypothesis fail to reject since null hypothesis very close to zero(i.e. the null model) 
- There is no association between price and the number of stay days 
